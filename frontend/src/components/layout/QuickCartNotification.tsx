...
export const QuickCartNotification: React.FC = () => {
    const navigate = useNavigate();
    const dispatch = useAppDispatch();
    const { lastAddedItem } = useAppSelector((state) => state.cart);
        ...
        <AnimatePresence>
            {lastAddedItem && (
                <motion.div
                    ...
                    animate={{ y: 0 }}
                    exit={{ y: '100%' }}
                    className="fixed bottom-20 left-4 right-4 z-[200] lg:bottom-8 lg:right-8 lg:left-auto lg:w-96"
                >
