...
async fn list(State(state): State<AppState>, jar: CookieJar) -> Result<Json<serde_json::Value>, AppError> {
    ...
    Ok(Json(backend_order::list_orders_for_user(&state.db, &user_id).await?))
}
...
async fn detail(
    ...
    let user_id = current_user_id(&state, &jar).await?;
    Ok(Json(backend_order::get_order_for_user(&state.db, &user_id, &id).await?))
}
...
async fn checkout(
    ...
    let user_id = current_user_id(&state, &jar).await?;
    Ok(Json(backend_order::checkout(&state.db, &user_id, input).await?))
}
