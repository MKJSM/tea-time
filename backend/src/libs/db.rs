...
pub async fn connect(config: &Config) -> Result<Pool, deadpool_postgres::CreatePoolError> {
...
pub async fn migrate(pool: &Pool) -> Result<(), tokio_postgres::Error> {
    ...
    let migration_dir = Path::new("./db/migration");
...
pub fn map_pool_error(error: deadpool_postgres::PoolError) -> tokio_postgres::Error {
    match error {
